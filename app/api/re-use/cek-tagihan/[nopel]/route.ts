...
export async function GET(req: NextRequest) { 
  try {
    ...
    const tgl : string = new Date().toLocaleDateString('en-ZA');

    const datatagihan : any[] = await prismadb.$queryRaw(
      Prisma.sql`call infotag_desk(${nopel},${tgl})` 
    )
    ...
    const resultTag : any[] = datatagihan.map(val => {   
      const res = {
        periode : val.f2 ,
        golongan : `${val.f8} - ${val.f9}` ,
        stanlalu : parseInt(val.f10),
        stanini : parseInt(val.f11),
        pakaim3 : parseInt(val.f12),
        hargaair : parseInt(val.f13),
        administrasi : parseInt(val.f15),
        pemeliharaan : parseInt(val.f16),
        retribusi : parseInt(val.f17),
        angsuran : parseInt(val.f19),
        denda : parseInt(val.f24)+parseInt(val.f25),
        materai : parseInt(val.f26),
        total : parseInt(val.f27)
      }
